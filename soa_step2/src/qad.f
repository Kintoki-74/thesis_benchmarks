...
c       dimension ql(nvar,max1dp1),    qr(nvar,max1dp1)
...
c                # from the cell corresponding  to q
  ...
  5          continue
       ...
       do 10 ivar = 1, nvar
c         ql(ivar,j-nghost+1) = valbig(ivar,nghost,j)
 ...
 10    continue
   ...
   24          continue
         ...
         do 25 ivar = 1, nvar
c 25         qr(ivar,lind) = qc1d(ivar,index)
 ...
 20    continue
       ...
       if (qprint) then
         ...
         do i=2,nc
c            write(dbugunit,4101) i,qr(1,i-1),ql(1,i)
...
c          # skip over sides 2 and 4 in this case
 ...
 205         continue
        ...
        do 210 ivar = 1, nvar
c            qr(ivar,i-nghost) = valbig(ivar,i,mjtot-nghost+1)
...
! 225         ql(ivar,lind+1) = qc1d(ivar,index)
 ...
 220    continue
       ...
       if (qprint) then
         ...
         do i=1,nr
c            write(dbugunit,4101) i,ql(1,i+1),qr(1,i)
...
c
 ...
 305         continue
        ...
        do 310 ivar = 1, nvar
c          qr(ivar,j-nghost) = valbig(ivar,mitot-nghost+1,j)
...
c                # preserves conservation in incompressible flow:
  ...
  324          continue
         ...
         do 325 ivar = 1, nvar
c 325         ql(ivar,lind+1) = qc1d(ivar,index)
...
c                # preserves conservation in incompressible flow:
 ...
 405         continue
        ...
        do 410 ivar = 1, nvar
c          ql(ivar,i-nghost+1) = valbig(ivar,i,nghost)
...
! 425         qr(ivar,lind) = qc1d(ivar,index)
 ...
 420    continue
       ...
       if (qprint) then
         ...
         do i=1,nr
c            write(dbugunit,4101) i, ql(1,i),qr(1,i)
            write(dbugunit,4101) i, ql(i,1),qr(i,1)
            enddo
       endif
       call rpn2(2,max1dp1-2*nghost,nvar,mwaves,maux,nghost,
